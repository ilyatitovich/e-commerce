...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginSchema) => {
    ...
    if (res.ok) {
      // Например, можно редиректить на главную или куда нужно
      window.location.href = "/";
    } else {
      const json = await res.json();
      setError(json.message || "Ошибка входа");
    }
  };
  ...
  return (
    <main className="flex items-center justify-center min-h-screen bg-gray-50">
      <form
        ...
        className="w-1/2 p-6 rounded-2xl shadow-md space-y-5 flex flex-col gap-4 bg-white"
      >
        ...
        >
          {/* Иконка Google (копия из регистрации) */}
          <svg
            className="w-5 h-5"
